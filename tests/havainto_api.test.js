...
describe("when there is initially some sightings saved", () => {
  ...
  describe("viewing a specific sighting", () => {
    test("succeeds with a valid id", async () => {
      ...
      const processedHavaintoToView = JSON.parse(
        JSON.stringify(havaintoToView)
      );
    ...
    test("fails with statuscode 404 if sighting does not exist", async () => {
      ...
      await api.get(`/api/lintudb/havainto/${validNonexistingId}`).expect(404);
    });

    test("fails with statuscode 400 id is invalid", async () => {
      const invalidId = "5a3d5da59070081a82a3445";

      await api.get(`/api/lintudb/havainto/${invalidId}`).expect(400);
    });
  });
  ...
  describe("addition of a new sighting", () => {
    ...
    test("fails with status code 400 if data invalid", async () => {
      ...
      await api.post("/api/lintudb/havainto").send(newHavainto).expect(400);
